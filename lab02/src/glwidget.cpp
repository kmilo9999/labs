...
void GLWidget::initializeGLFragmentShaders() {
    ...
    std::vector<float> coordinates = {
        -0.5f,  0.5f,  0.0f, 0.8f,0.0f,0.0f, 0,0,
        -0.5f, -0.5f,  0.0f, 0.0f,0.8f,0.0f, 0,1,
         0.5f,  0.5f,  0.0f, 0.0f,0.0f,0.8f, 1,0,
         0.5f, -0.5f,  0.0f, 0.0f,0.8f,0.0f, 1,1
    };
...
//    std::vector<float> coordinates_colors = {
...
void GLWidget::paintGLTransformationsVertexShaders() {
    // Clear the color and depth buffers.
        ...
        glUniform3f(glGetUniformLocation(m_program, "color"), 0.5, 0.9, 0.1);
        ...
        glm::mat4 trans;
        trans = glm::translate(trans,glm::vec3(0,0.5 + fabs( sin(3 * time) ),0));

        glUniformMatrix4fv( glGetUniformLocation(m_program, "model"), 1, GL_FALSE, glm::value_ptr(trans));
        ...
        glUniform3f(glGetUniformLocation(m_program, "color"), 0.5, 0.25, 0.5);

        // TODO: Scale the square x2. (Task 7)
        //glm::mat4 scale;
        glm::mat4 squareModelM;
        squareModelM = glm::scale(squareModelM,glm::vec3(2,2,2));


        // TODO: Rotate the square to lie flat on the XZ plane. (Task 7)
        //glm::mat4 rotation;
        squareModelM = glm::rotate(squareModelM,90.0f,glm::vec3(1,0,0));
        squareModelM = glm::rotate(squareModelM,time,glm::vec3(0,0,1));
        glUniformMatrix4fv( glGetUniformLocation(m_program, "model"), 1, GL_FALSE, glm::value_ptr(squareModelM));
...
void GLWidget::paintGLFragmentShaders() {
    ...
    switch (settings.shaderProgram) {
        case SOLID_SHADER_PROGRAM:
            ...
            glUniform3f(glGetUniformLocation(m_solidProgramID, "color"), 0.5, 0.9, 0.5);
